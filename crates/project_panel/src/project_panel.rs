...
/// The file nesting assignment for one worktree, computed among the visible
/// file children of each directory. The assignment is flat: a nested file has
/// exactly one parent file and never has children of its own.
#[derive(Debug, Default)]
struct FileNesting {
    /// Maps a nested file to the sibling file it nests under.
    child_to_parent: HashMap<ProjectEntryId, ProjectEntryId>,
    /// Files that have at least one nested child, visible or not.
...
impl ProjectPanel {
    ...
    fn collapse_entry(
        ...
        // A nested file collapses the file it nests under, like a file in a
        // directory collapses that directory.
                ...
                .background_spawn(async move {
                    for worktree_snapshot in visible_worktrees {
                        ...
                        while let Some(entry) = entry_iter.entry() {
                            ...
                            // File nesting is computed per directory, over all
                            // of the directory's visible file children, when
                            // the first file of that directory is reached.
                            ...
                            {
                                // A nested file is visible only while the file
                                // it nests under is expanded.
    ...
    fn expand_entry(
        ...
        self.project.update(cx, |project, cx| {
                ...
                .zip(self.state.expanded_dir_ids.get_mut(&worktree_id))
            {
                ...
                if let Some(mut entry) = worktree.entry_for_id(entry_id) {
                    // A nested file is visible only while the file it nests
                    // under is expanded, so expand that file as well.
...
/// Moves nested files so that they immediately follow the file they nest
/// under, wherever the sort order placed that file.
