//! Matches the file nesting patterns from the `file_nesting` setting against
//! the file names of a directory, producing a flat parent → children
//! assignment: every file has at most one parent, and a nested file never has
//! children of its own.
...
/// Compiles a pattern like `*.ts` into a regex, turning each `*` into `(.*)`
/// when capturing (so `${capture}` can reference it) or `.*` otherwise.
